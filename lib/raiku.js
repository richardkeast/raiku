	...
	respondToHaikus(message){
		   ...
		   !isBotMessage(message))
		{
			...
			for (i = 0; i < words.length; ++i) {
				line1 = line1 + words[i] + " ";
				line1Syllables += this.syllable(words[i]);
				if (line1Syllables == 5)
				{
					var line2Syllables = 0;
					for (i2 = i + 1; i2 < words.length; ++i2) {
						line2 = line2 + words[i2] + " ";
						line2Syllables += this.syllable(words[i2]);
						if (line2Syllables == 13)
						{
							var line3Syllables = 0;
							for (i3 = i2 + 1; i3 < words.length; ++i3) {
								line3 = line3 + words[i3] + " ";
								line3Syllables += this.syllable(words[i3]);
